...
try:
    ...
    raise SystemExit(
        ...
        "  .venv/bin/python src/experiments/222/test_graph_isomorphism_222_single_orbit.py"
    ) from exc
...
def describe_augmented_graph(node_bits, edge_orbits):
    """返回图结构：节点ID->state、虚拟节点代表的原始边、以及边列表。"""
...
def sample_outside_permutations(group_elems, sample_size, seed=222):
...
def main():
    ...
    outside_perms = sample_outside_permutations(group_elems, sample_size=50, seed=222)
    outside_iso_flags = []
    outside_iso_perms = []
