...
if __package__ in (None, ""):
    ...
    from geometry import generate_points_422
...
def classify_hard_mask(
    ...
    if not validation["valid"]:
        return None
    validation["validation_tier"] = "facet" if validation.get("is_facet") else "candidate_supporting_face"
    if reference is not None:
        validation.update(classify_discovered_plane(validation["normal"], validation["offset"], reference))
    else:
        validation.update(classify_discovered_plane(validation["normal"], validation["offset"], reference))
    if not validation.get("is_facet"):
        validation["tier"] = "unknown_supporting_face" if validation.get("canonical_key") else "candidate_supporting_face"
    return validation
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Geometric-DiffUCO inference for the 4-2-2 Bell polytope.")
...
def main() -> None:
    ...
    points = generate_points_422().to(device)
