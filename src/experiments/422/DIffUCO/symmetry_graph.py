...
if __package__ in (None, ""):
    from geometry import generate_states_422
else:
    from .geometry import generate_states_422
...
def _generator_definitions() -> List[tuple[str, Callable[[State], State]]]:
    idx: Dict[str, int] = {"A0": 0, "A1": 1, "B0": 2, "B1": 3, "C0": 4, "C1": 5, "D0": 6, "D1": 7}
    ...
    return [
        ...
        ("FlipOut_D0", lambda state: flip(state, "D0")),
        ("FlipOut_D1", lambda state: flip(state, "D1")),
    ]


def build_symmetry_graph() -> SymmetryGraph:
    states = generate_states_422()
