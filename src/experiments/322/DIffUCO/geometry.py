...
def generate_states_322() -> List[tuple[int, ...]]:
        ...
        for a1 in (1, -1):
            for b0 in (1, -1):
                for b1 in (1, -1):
                    for c0 in (1, -1):
                        for c1 in (1, -1):
                            states.append((a0, a1, b0, b1, c0, c1))
    return states


def generate_points_322() -> torch.Tensor:
    rows: List[List[float]] = []
    for a0, a1, b0, b1, c0, c1 in generate_states_322():
        rows.append(
            [
                a0,
                a1,
...
def support_violation(points: torch.Tensor, normal: torch.Tensor, offset: torch.Tensor, margin: float = 1e-4) -> torch.Tensor:
    signed_distance = points @ normal.transpose(0, 1) + offset.unsqueeze(0)
    positive = torch.relu(signed_distance - margin).pow(2).mean(dim=0)
    negative = torch.relu(-signed_distance - margin).pow(2).mean(dim=0)
    ...
    result.update(
        {
            ...
            "offset": offset,
            "support": side,
            "valid": plane_value < plane_eps and side["closer_side"] == 0,
        }
    )
    if not result["valid"]:
