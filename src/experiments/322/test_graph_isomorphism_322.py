...
try:
    ...
    raise SystemExit(
        ...
        "  .venv/bin/python src/experiments/322/test_graph_isomorphism_322.py"
    ) from exc
...
def build_augmented_graph_structure(node_bits, edge_orbits):
    ...
    partition = [bit0, bit1]
    node_id_to_state = {i: ID_TO_STATE[i] for i in range(N_VERTICES)}
    ...
    for orbit in edge_orbits:
        ...
        for u, v in orbit:
            ...
            edges.append((v, w))
            cls.add(w)
            node_id_to_state[w] = None
            virtual_node_to_edge[w] = (u, v)
        partition.append(cls)

    return {
        "total_nodes": next_virtual,
        "node_id_to_state": node_id_to_state,
        "virtual_node_to_edge": virtual_node_to_edge,
        "edges": edges,
        "partition": partition,
    }


def describe_augmented_graph(node_bits, edge_orbits):
    return build_augmented_graph_structure(node_bits, edge_orbits)
...
def sample_outside_permutations(group_elems, sample_size, seed=322):
...
def main():
    ...
    outside_perms = sample_outside_permutations(group_elems, sample_size=50, seed=322)
    outside_iso_flags = []
    outside_iso_perms = []
    ...
    print(f"各边轨道大小: {[len(orbit) for orbit in edge_orbits]}")
    ...
    if all_gens_iso and all_outside_noniso:
        print("结论: 该多边色图通过了“群内全同构 + 群外50个全不同构”检验。")
    elif all_gens_iso:
        print("结论: 群内全同构成立，但群外50个并非全部不同构。")
    else:
        print("结论: 该多边色图未通过群内全同构检验。")
