...
class HotspotProfile:
    protected_remove: set[int]
    protected_add: set[int]
    soft_remove_scores: dict[int, float]
    soft_add_scores: dict[int, float]
...
class SearchState:
    ...
    remove_hotspots_kept: int
    add_hotspots_kept: int
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bridge projection using reverse-collapse hotspots as hard anti-collapse constraints.")
    ...
    parser.add_argument("--top-remove", type=int, default=2)
    parser.add_argument("--top-add", type=int, default=2)
    parser.add_argument("--top-swap", type=int, default=2)
    parser.add_argument("--hotspot-frequency-threshold", type=float, default=0.08)
    parser.add_argument("--energy-weight", type=float, default=1.0)
    parser.add_argument("--escape-weight", type=float, default=4.0)
    parser.add_argument("--source-similarity-weight", type=float, default=2.0)
    parser.add_argument("--support-drop-weight", type=float, default=1.0)
    parser.add_argument("--protected-retention-weight", type=float, default=8.0)
    parser.add_argument("--soft-hotspot-weight", type=float, default=2.0)
    parser.add_argument("--exact-bonus", type=float, default=1.0)
    parser.add_argument("--exact44-penalty", type=float, default=10.0)
    parser.add_argument("--min-protected-retention", type=float, default=0.75)
...
def load_hotspot_profile(path: Path, threshold: float) -> HotspotProfile:
    payload = json.loads(path.read_text(encoding="utf-8"))
    protected_remove: set[int] = set()
    protected_add: set[int] = set()
    soft_remove_scores: dict[int, float] = {}
    soft_add_scores: dict[int, float] = {}
    for class_item in payload["target_classes"]:
        for item in class_item.get("top_remove_indices", []):
            index = int(item["index"])
            frequency = float(item["frequency"])
            soft_remove_scores[index] = max(soft_remove_scores.get(index, 0.0), frequency)
            if frequency >= threshold:
                protected_remove.add(index)
        for item in class_item.get("top_add_indices", []):
            index = int(item["index"])
            frequency = float(item["frequency"])
            soft_add_scores[index] = max(soft_add_scores.get(index, 0.0), frequency)
            if frequency >= threshold:
                protected_add.add(index)
    return HotspotProfile(
        protected_remove=protected_remove,
        protected_add=protected_add,
        soft_remove_scores=soft_remove_scores,
        soft_add_scores=soft_add_scores,
    )
...
def evaluate_state(
    *,
    ...
    kept_remove = sum(1 for index in hotspot_profile.protected_remove if index < len(hard) and int(hard[index]) == 0)
    kept_add = sum(1 for index in hotspot_profile.protected_add if index < len(hard) and int(hard[index]) == 1)
    total_protected = len(hotspot_profile.protected_remove) + len(hotspot_profile.protected_add)
    protected_retention_ratio = 1.0 if total_protected == 0 else (kept_remove + kept_add) / total_protected

    soft_hotspot_score = 0.0
    for index, weight in hotspot_profile.soft_remove_scores.items():
        if index < len(hard) and int(hard[index]) == 0:
            soft_hotspot_score += weight
    for index, weight in hotspot_profile.soft_add_scores.items():
        if index < len(hard) and int(hard[index]) == 1:
            soft_hotspot_score += weight
    ...
    objective = (
        ...
        + args.protected_retention_weight * protected_retention_ratio
        + args.soft_hotspot_weight * soft_hotspot_score
        + exact_bonus
        - exact44_penalty
    )
    return SearchState(
        ...
        remove_hotspots_kept=int(kept_remove),
        add_hotspots_kept=int(kept_add),
        protected_retention_ratio=float(protected_retention_ratio),
...
def action_allowed(mask: torch.Tensor, action: dict[str, int], profile: HotspotProfile) -> bool:
    if action["type"] == "remove":
        return action["index"] not in profile.protected_add
    if action["type"] == "add":
        return action["index"] not in profile.protected_remove
    if action["type"] == "swap":
        return action["remove_index"] not in profile.protected_add and action["add_index"] not in profile.protected_remove
    return True
...
def propose_neighbors(
    ...
    *,
    ...
    def maybe_append(candidate_mask: torch.Tensor, action: dict[str, int], bucket: list[tuple[float, SearchState]]) -> None:
        if not action_allowed(mask, action, hotspot_profile):
            return
        ...
        if evaluated.protected_retention_ratio + 1e-8 < args.min_protected_retention:
            return
    ...
    for remove_index in remove_pool:
        for add_index in add_pool:
            ...
            maybe_append(candidate, {"type": "swap", "remove_index": remove_index, "add_index": add_index}, swap_candidates)
        ...
        path.append(
            {
                ...
                "remove_hotspots_kept": int(state.remove_hotspots_kept),
                "add_hotspots_kept": int(state.add_hotspots_kept),
                "protected_retention_ratio": float(state.protected_retention_ratio),
...
def main() -> None:
    ...
    args.reverse_collapse = normalize_cli_path(args.reverse_collapse)
    args.output = normalize_cli_path(args.output)

    device = torch.device(args.device)
    jump_bank = json.loads(args.jump_bank.read_text(encoding="utf-8"))
    hotspot_profile = load_hotspot_profile(args.reverse_collapse, threshold=args.hotspot_frequency_threshold)
            ...
            results.append(
                {
                    ...
                    "protected_remove_hotspots": sorted(hotspot_profile.protected_remove),
                    "protected_add_hotspots": sorted(hotspot_profile.protected_add),
    ...
    payload = {
        "summary": {
            ...
            "reverse_collapse": str(args.reverse_collapse),
            ...
            "hotspot_frequency_threshold": float(args.hotspot_frequency_threshold),
            "min_protected_retention": float(args.min_protected_retention),
            "num_protected_remove_hotspots": len(hotspot_profile.protected_remove),
            "num_protected_add_hotspots": len(hotspot_profile.protected_add),
        },
        "results": results,
    }
